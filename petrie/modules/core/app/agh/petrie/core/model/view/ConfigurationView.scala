...
case class ElementToClickView(selector: SelectorConfigurationView, actionType: String)
  extends PreScrapingConfigurationElementView
case class WaitTimeoutView(timeout: Int, actionType: String) extends PreScrapingConfigurationElementView
case class ScrollToElementView(selector: SelectorConfigurationView, actionType: String)
  extends PreScrapingConfigurationElementView
case class WriteToElementView(selector: SelectorConfigurationView, text: String, actionType: String)
  extends PreScrapingConfigurationElementView
...
object ConfigurationView {
  ...
  val preScrapingConfigurationElementViewReads: Reads[PreScrapingConfigurationElementView] =
    (__ \ "actionType").read[String].flatMap {
      case c if c == extractClassName(ElementToClickView) =>
        Json.reads[ElementToClickView].asInstanceOf[Reads[PreScrapingConfigurationElementView]]
      case c if c == extractClassName(WaitTimeoutView) =>
        Json.reads[WaitTimeoutView].asInstanceOf[Reads[PreScrapingConfigurationElementView]]
      case c if c == extractClassName(ScrollToElementView) =>
        Json.reads[ScrollToElementView].asInstanceOf[Reads[PreScrapingConfigurationElementView]]
      case c if c == extractClassName(WriteToElementView) =>
        Json.reads[WriteToElementView].asInstanceOf[Reads[PreScrapingConfigurationElementView]]
    }
